import Food, { FoodArgs } from "./Food.js";
import Snake, { SnakeArgs } from "./Snake.js";
...
export default class GameController {
  ...
  run() {
    ...
    this.snake.setY(Y);
    // 检测是否吃到食物
    let checkEat: boolean = this.checkEat();
    if (checkEat) {
      this.updateScore();
      this.updateLevel();
  ...
  checkEat() {
    return (
      this.snake.head_X === this.food.X && this.snake.head_Y === this.food.Y
    );
  }
  updateScore() {
